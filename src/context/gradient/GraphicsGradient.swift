 ...
 * - Parameter: transformation: we need transformation as this is the only way to achieve the squashed gradient look
 */
public struct GraphicsGradient:GraphicsGradientKind{
    ...
    init(_ colors:[CGColor] = [], _ locations:[CGFloat] = [], _ transformation:CGAffineTransform? = nil){
        self.colors = colors
        let locs = locations.isEmpty ? CGFloatParser.interpolatedRange(0, 1, colors.count) : locations//- Fixme: ⚠️️ ⚠️️ add support for nil aswell
        self.locations = locs
