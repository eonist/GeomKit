...
class Arc:IArc{
     ...
     * - Fixme: ⚠️️ ⚠️️ the reason Arc4 exists is because it can work with axises that are 0, if you want to Make Arc3 the primary arc class again you need to make sure it can also work with axis that are 0, the benefit of Ar3 is that its absolute precis while editing the arc instance start and end positions.
     * - Fixme: ⚠️️ ⚠️️ a largeflag two that indicates wheter the start point is on the left or right side when one of the axises is 0 ould be a solution to the problem with 0 axises
     * - Fixme: ⚠️️ ⚠️️ you might also want to consider including a position argument, for better performance and ease. this is just an idea
     */
    var start: CGPoint
    var xRadii: CGFloat// :- Fixme: ⚠️️ i think the xRadii and yRadii can't be negative, confirm please and write it up as a note
    var yRadii: CGFloat
